...
from absorb.core.tasks.commands import (
    tasks,
    parse_date,
    load_json,
)
...
def test_parse_dates_plus_with_character() -> None:
    """A plus sign (+) with invalid characters should return the current
    time."""
    result = parse_date("+1")
    assert result.replace(microsecond=0) == datetime.now().replace(microsecond=0)
...
def test_parse_dates_only_day() -> None:
    """A plus sign (+) with a 'd' character should return the amount of days
    being added to the current time."""
...
def test_parse_dates_day_with_hours() -> None:
    """A plus sign (+) with 'd' and 'h' characters should return the amount of
    days and hours being added to the current time."""
...
def test_parse_dates_day_with_hours_minutes() -> None:
    """A plus sign (+) with 'd', 'h' and 'm' characters should return the
    amount of days, hours and minutes being added to the current time."""
...
def test_parse_dates_day_with_hours_minutes_seconds() -> None:
    """A plus sign (+) with 'd', 'h', 'm' and 's' characters should return the
...
def test_parse_dates_only_day_args() -> None:
    """A plus sign (+) with a 'd' character should return the amount of days
    being added to the give time (args)."""
    result = parse_date(
        "+1d",
...
def test_task_no_file(runner: CliRunner) -> None:
    """Adding a task normally should return 0, but if a file is not found, it
    should return -1."""
    result = runner.invoke(tasks, ["add", "New task.", ".", "low", "."])
    assert result.exit_code == 0


def test_task_show_group_valid_hour(
    runner: CliRunner,
) -> None:
    """Adding a task with duedate in 5 hours should return 0, if the group is
    valid in nature."""
    # mock entry
...
def test_task_show_group_valid(runner: CliRunner) -> None:
    """Showing a task (by group) should return 0, if the group is valid in
    nature."""
    # mock entry
...
def test_task_show_group_invalid(runner: CliRunner) -> None:
    """Showing a task (by group) should return -1, if the group is invalid in
    nature."""
    result = runner.invoke(tasks, ["show-group", "not a tag"])
    assert result.exit_code == -1


def test_task_show_group_valid_FileNotFoundError(
    runner: CliRunner,
) -> None:
    """Showing a task (by group) but task.json doesn't exist it should return.
...
def test_task_show_FileNotFoundError(
    runner: CliRunner,
) -> None:
    """The show command for tasks but tasks.json doesn't exist should return.
...
def test_task_add_with_date_and_groups(
    runner: CliRunner,
) -> None:
    """Adding a task with a date and a group should return 0, if it is valid in
    nature."""
...
def test_task_edit(runner: CliRunner) -> None:
    ...
    result = runner.invoke(
        tasks,
        ["edit", "#1", "Modified task.", ".", "low", "."],
    )
    assert result.exit_code == 0


def test_task_edit_FileNotFoundError(
    runner: CliRunner,
) -> None:
    """Editing a task normally but tasks.json doesn't exist should return 0, if
...
def test_task_edit_with_date_and_groups(
    runner: CliRunner,
) -> None:
    """Editing a task with a date and groups should return 0, if it is valid in
    nature."""
    result = runner.invoke(
        tasks,
        [
...
def test_task_delete(runner: CliRunner) -> None:
    """Deleting a task with the help of a ID should return 0, if it is valid in
    nature."""
    result = runner.invoke(tasks, ["delete", "#1"])
    assert result.exit_code == 0


def test_task_delete_FileNotFoundError(
    runner: CliRunner,
) -> None:
    """Deleting a task with the help of a ID but tasks.json doesn't existshould
    return 0, if it is valid in nature."""
    os.rename(ROOT_PATH / "tasks.json", ROOT_PATH / "_tasks.json")
    result = runner.invoke(tasks, ["delete", "#1"])
    assert result.exit_code == 0
    os.remove(ROOT_PATH / "tasks.json")
